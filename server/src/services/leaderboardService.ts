...
class LeaderboardService {
   ...
   */
  public async generateLeaderboard(challengeId: number): Promise<any> {
    try {
      ...
      const entries = await ChallengeEntry.findAll({
        where: {
          challengeId,
          disqualified: false,
          status: { [Op.in]: ['active', 'completed'] } // Only include active or completed entries
        },
        include: [
          {
            model: User,
   ...
   */
  private async applyRankingAlgorithm(entries: ChallengeEntry[], challenge: Challenge): Promise<ChallengeEntry[]> {
   ...
   */
  private async updateEntryRanks(rankedEntries: ChallengeEntry[]): Promise<void> {
   ...
   * Format leaderboard for response
   */
  private formatLeaderboard(rankedEntries: ChallengeEntry[], challenge: Challenge): any {
    const leaderboardEntries = rankedEntries.map((entry, index) => {
      return {
        ...
        discordUsername: entry.user?.discordUsername || null,
        metrics: {
   ...
   */
  private broadcastLeaderboardUpdate(challengeId: number, leaderboard: any): void {
    if (socketService) {
      try {
        // Emit to challenge-specific room
        socketService.to(`challenge-${challengeId}`).emit('leaderboard-update', leaderboard);
        
        // Also emit to admin room
        socketService.to('admin').emit('leaderboard-update', leaderboard);
   ...
   */
  public async updateLeaderboardAfterTrade(challengeEntryId: number): Promise<void> {
    try {
   ...
   */
  public async getTopPerformers(challengeId: number, topPercent: number = 0.3): Promise<any[]> {
    try {
      // Generate updated leaderboard
   ...
   * @param challengeId Challenge ID
   */
  public async getDisqualifiedEntries(challengeId: number): Promise<any[]> {
    try {
      // Get all disqualified entries for this challenge
      const disqualifiedEntries = await ChallengeEntry.findAll({
        where: {
          challengeId,
          disqualified: true
        },
   ...
   */
  public async getEntriesAtRisk(challengeId: number, thresholdPercentage: number = 0.9): Promise<any[]> {
    try {
      // Get challenge rules
      const challenge = await Challenge.findByPk(challengeId);
      if (!challenge) {
