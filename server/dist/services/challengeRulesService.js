...
class ChallengeRulesService {
     ...
     */
    async validateRules(challengeEntryId) {
        try {
     ...
     */
    async checkMaxDrawdown(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    async checkDailyDrawdown(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    async checkMaxRiskPerTrade(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    async checkMinTradeRequirements(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    async checkMinTradeDuration(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
        // If no challenge or no trades, no violations
        if (!challenge || trades.length === 0) {
     ...
     */
    async checkForHedging(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    tradesOverlap(trade1, trade2) {
        return ((trade1.entryTime <= trade2.exitTime && trade1.exitTime >= trade2.entryTime) ||
            (trade2.entryTime <= trade1.exitTime && trade2.exitTime >= trade1.entryTime));
    }
    /**
     * Check if trades are in opposite directions
     */
    areOppositeDirections(trade1, trade2) {
     ...
     */
    async checkTradingDays(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     */
    async checkSwingTradingRequirement(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
     ...
     * Check consistency rule
     */
    async checkConsistencyRule(entry, trades) {
        const violations = [];
        const challenge = entry.challenge;
        ...
        if (challenge.type === 'weekly') {
            ...
            const validTrades = trades.filter(trade => trade.lotSize >= avgLotSize * 0.1 && trade.duration >= minTradeDurationSeconds);
...
exports.default = new ChallengeRulesService();
//# sourceMappingURL=challengeRulesService.js.map